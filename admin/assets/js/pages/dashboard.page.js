...
const DashboardPage = {
  ...
  init() {
    ...
    document.getElementById('bootstrapFirestoreBtn')?.addEventListener('click', async (e) => {
      if (!window.DatabaseBootstrapService) return;

      const manter = confirm(
        'Inicializar Firestore agora?\n\nOK = manter documentos já existentes\nCancelar = escolher sobrescrever na próxima confirmação.'
      );

      const overwrite = !manter
        ? confirm('Sobrescrever documentos base existentes?\n\nOK = sobrescrever\nCancelar = cancelar operação')
        : false;

      if (!manter && !overwrite) return;

      const button = e.currentTarget;
      const original = button.querySelector('span:last-child')?.textContent || 'Inicializar Banco';
      button.disabled = true;
      const labelNode = button.querySelector('span:last-child');
      if (labelNode) labelNode.textContent = 'Inicializando...';

      try {
        const result = await DatabaseBootstrapService.seed({ overwrite });
        if (!result.success) {
          throw new Error(result.error || 'Falha ao inicializar banco');
        }
        ...
        alert(
          ...
          `config: ${s.config || 0}\n` +
          `tipos_produto: ${s.tipos_produto || 0}\n` +
          `categorias: ${s.categorias || 0}\n` +
          `marcas: ${s.marcas || 0}\n` +
          `banners: ${s.banners || 0}\n` +
          `menu_links: ${s.menu_links || 0}\n` +
          `redes_sociais: ${s.redes_sociais || 0}\n` +
          `paginas: ${s.paginas || 0}`
        );
      } catch (error) {
        alert(`Erro ao inicializar banco: ${error.message || error}`);
      } finally {
