        ...
        if (user.email) {
          ...
          if (!snap.empty) {
            ...
            if (existing.id !== user.uid) {
              try {
                // Em convites por email, o usuário comum pode não ter permissão para apagar.
                // A migração para /usuarios/{uid} deve continuar mesmo assim.
                await firebaseDb.collection(this.collection).doc(existing.id).delete();
              } catch (deleteError) {
                console.info('Migração de perfil: doc legado mantido (sem permissão para deletar).');
              }
            }
          }
        }
